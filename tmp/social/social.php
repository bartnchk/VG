...
class PlgAuthenticationSocial extends JPlugin
{
	public function onUserAuthenticate($credentials, $options, &$response)
	{
		...
		{
			$db = JFactory::getDbo();
			$user = $db->setQuery("SELECT * FROM #__z_users WHERE facebook_id = " . $credentials['fb_id'])->loadObject();

		}
		elseif (!empty($credentials['google_id']))
		{
			$db = JFactory::getDbo();
			$user = $db->setQuery("SELECT * FROM #__z_users WHERE gmail_id = " . $credentials['google_id'])->loadObject();
		}
		else {
