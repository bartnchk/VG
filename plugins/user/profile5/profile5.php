 ...
 JLoader::register('OptimizeHelper', JPATH_ADMINISTRATOR . '/components/com_plants/helpers/optimize.php');
  ...
  class plgUserProfile5 extends JPlugin
  {
	...
	function onUserAfterSave($data, $isNew, $result, $error)
	{
	    ...
	    $filename = $this->storeUserPhoto();
        if($filename)
            $data['profile5']['photo'] = $filename;
        else
            $data['profile5']['photo'] = $this->getUserPhoto($userId);
	...
	public function storeUserPhoto()
    {
        $input = JFactory::getApplication()->input;
        $image = $input->files->get('jform');

        if(!$image['profile5']['photo']['size'])
            return false;

        $filename = uniqid() . $image['profile5']['photo']['name'];
        $tmp = $image['profile5']['photo']['tmp_name'];
        $folder = 'images/user_photos/';

        OptimizeHelper::optimizePhoto($tmp, 500, 300, $folder, $filename);
